...
cd $RESULTDIR
bash $PHYLOGENOMICS/pipelines/bam_to_plastome_vcf.sh $OUTNAME.1.txt
cd ..

# convert the vcfs to fasta:
gawk -F "\t" 'NR > 1 {print $1".vcf"}' $INFILE > $OUTNAME.2.txt
perl $PHYLOGENOMICS/converting/vcf2fasta.pl -samples $OUTNAME.2.txt -output $OUTNAME -thresh 0 -cov 300

# trim missing data at the 0.1 missing threshold:
perl $PHYLOGENOMICS/parsing/trim_missing.pl -in $OUTNAME.fasta -out $OUTNAME.trimmed.fasta -row 0.1 -col 0.1
...
perl $PHYLOGENOMICS/reporting/make_kml.pl -i $OUTNAME.locs.txt -c species_colors.txt -o $OUTNAME
...
perl $PHYLOGENOMICS/converting/relabel_samples.pl -i $OUTNAME.trimmed.fasta -label $OUTNAME.rename.locs -out $OUTNAME.locs.fasta

#convert to nexus:
perl $PHYLOGENOMICS/converting/convert_file.pl $OUTNAME.locs.fasta $OUTNAME.locs.nex
