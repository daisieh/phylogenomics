...
foreach my $aln (@gene_alns) {
	my $rel_exec = Bio::Tools::Run::Phylo::Hyphy::BatchFile->new(-params => {'bf' => "/Users/daisie/Documents/Work/Sandbox/hyphy/examples/LRT.bf", 'order' => ["Universal", "Custom", $aln, "001001", $firsttree]});
	my $name = $aln->description();
	my $resultstr = $name;
 	$rel_exec->alignment($aln);
	$rel_exec->set_parameter('3', "012012");
 	if ($trees{$name} == undef) {
 	...
 	$rel_exec->outfile_name("$output_name"."_$name.bfout");
